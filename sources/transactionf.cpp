...
void install(const std::string &url, const bool force, const bool file) {
  ...
  if (!force) {
    if (!clif::confirm("Are you sure you want to install the library from '" +
                       url + "'")) {
...
void remove(const std::string &pkgName, bool force) {
  ...
  if (!force) {
    ...
    if (!clif::confirm("Are you sure you want to remove '" + pkgName +
                       "' library?")) {
...
void type() {
  ...
  if (buildData.find("@") == buildData.end()) {
    clif::log(FATAL,
              "metadata missing key: type (@) (try changing the project type)");
    return;
  }

  if (std::get<std::string>(buildData["@"]) == "# @project") {
    clif::log(INFO, "project");
  } else if (std::get<std::string>(buildData["@"]) == "# @library") {
    clif::log(INFO, "library");
  } else {
    clif::log(FATAL,
...
void use_template(const std::string &name,
  ...
  if (!name.empty() &&
    ...
    clif::log(INFO, "template successfully created: " +
                        (targetDirectory / name).string());
  } else {

    clif::log(INFO, "templates:");
    for (const auto &entry :
         fs::directory_iterator(HOME_DIRECTORY / "_sys" / "templates")) {
